      ...
      if ($request->has('start_date') && $request->has('end_date')) {
        ...
        foreach ($dataTransaction as $item) {
            ...
            $data[] = [
                ...
                'jenis' => $jenis, // 'jenis' => 'E-Commerce' or 'Kasir
        ...
        $total = [
            'total_harga_jual' => $collection->sum('harga_jual'),
            'total_harga_beli' => $collection->sum('harga_beli'),
            'total_laba_rugi' => $collection->sum('total_laba_rugi'),
        ];

        $total = collect($total);

        return view('pages._Main.Accounting.ProfitLoss.index', [
            'data' => $collection,
            'total' => $total,
        ]);
    }
