        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="pt-4">
                        <form action="{{ route('admin.rules.update', $rule->id) }}"
                              method="POST">
                            @csrf
                            @method('PUT')
                            <div class="row">
...
@section('script')
    ...
    <script>
        const dataPestDiseases = @json($pestDiseases).map((item) => {
            const selected = item.id === @json($rule).pest_disease_id ? true : false;
            return {
                id: item.id,
        ...
        const dataSymptoms = @json($symptoms).map((item) => {
            const selected = @json($selectedSymptoms).includes(item.id) ? true : false;
            return {
                id: item.id,
                text: `${item.code} - ${item.label}`,
                selected: selected
